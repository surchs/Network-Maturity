...
def ParamEst(trainData, trainLabel, numberCores):
    ...
    parameters = {'C': np.arange(1, 1000, 10).tolist()}
...
def TestModel(trainModel, testData, testLabel, cv=0, bestC=1, bestE=0.1):
...
def Processer(feature,
    ...
    # we need to get the an index of the training and the testing data
    # cvObject = LeaveOneOut(feature.shape[0])
    kObject = KFold(feature.shape[0], 10, shuffle=True)

    # prepare an array of predicted ages to store each iterations prediction
    predictedAges = np.zeros_like(label)
    trueAges = np.zeros_like(label)
    # and a list of rmse so I can average them over all iterations
    rmseList = []
    # and lastly a runtime varialbe
    run = 1

    # now we need to loop over the index
    for trainIndex, testIndex in kObject:
        print '\nRunning LOOCV iteration number:', str(run)
        # make training and testing dataset
        trainData = feature[trainIndex, ...]
        trainLabel = label[trainIndex, ...]
        testData = feature[testIndex, ...]
        testLabel = label[testIndex, ...]

        # get the correct number of features
        print 'Feature Selection for LOOCV run', str(run)
        (newFeature,
         featNo,
         newFeatNo,
         featureScore,
         ...
         featureIndex) = FeatureSelection(trainData, trainLabel)
        print 'Feature Selection is done. '
        print 'Old noFeat:', str(featNo), 'new noFeat:', str(newFeatNo)

        # now estimate the best parameters for this iteration
        if estParameters == 1:
        ...
        # train a model with these best parameters
        trainModel = TrainModel(trainData, trainLabel, bestC, bestE)

        # and predict the left out subject
        ...
        predictedAges[testIndex, ...] = predKeep
        trueAges[testIndex, ...] = trueKeep
    ...
    return (trueAges, predictedAges, trainModel, featNo, newFeatNo, rmse)
...
def Main(preprocDict,
    ...
    for network in networkList:
        ...
        if feature.shape[1] > 1 and not np.isnan(feature.max()):
            print '\n\nRunning network', str(network), 'now. '

            # store the output in the dictionary again
            machineDict[network] = Processer(feature,
                                             labels,
                                             numberCores,
                                             estParameters,
                                             givenC,
            ...
            print 'or has a nan value inside :O '
            print 'Anyways, this makes no sense for this type of anlysis so  '
