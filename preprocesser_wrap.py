...
Todo:
    ...
    - think about whether the flexibility of the script should be tailored
      towards the filenames of subList.txt and configfile.txt, towards the
...
def Averager(masked, stackArray=np.array([], dtype='float32')):
    ...
    # an increasing array if the function is called multiple timeSeries
    ...
    if stackArray.size == 0:
        stackArray = average[np.newaxis, ...]
    else:
        stackArray = np.concatenate((stackArray,
                                       average[np.newaxis, ...]), axis=0)

    return ((average, stackArray))


def Correlater(stackArray, feature=np.array([], dtype='float32')):
    ...
    # and stacks vectors into an array if called multiple timeSeries

    corr = np.corrcoef(stackArray)
...
def SubjectProcesser(arguments):
    ...
    # for every unique mask value (because we can't assume that the values
    # are are continues from 1 to max_value(mask))
    for num in np.unique(maskData[maskData != 0]):
        # switch for single voxel / node operations
        # single voxels operationsare based on (downsampled) voxels in the
        # brain that build the connectome (usually takes long since the
        # number of elements is much bigger)
        if nodeMode == 1:
            # average the mask that was just run because we want just one
            # value per mask
            (outMask, smallMask) = Masker(funcData, maskData, num)
            (avgerage, stackArray) = Averager(smallMask,
                                              stackArray=stackArray)

        else:
            # find the coordinates of voxels in the mask
            index = np.where(maskData == num)
            # number of voxels in the mask
            numberVoxels = len(index[0])
            xPos = index[0]
            yPos = index[1]
            zPos = index[2]
            for pos in np.arange(numberVoxels):
                timeSeries = funcData[xPos[pos], yPos[pos], zPos[pos]]
                if stackArray.size == 0:
                    stackArray = timeSeries[np.newaxis, ...]
                else:
                    stackArray = np.concatenate((stackArray,
                                                 timeSeries[np.newaxis, ...]),
                                                axis=0)

    # when the average is built over all nodes in the mask, we calculate
    # the correlation matrix and output it into the variable 'corr' for
    # each subject. The lower triangle of this matrix is the feat vector.
    # the feat vectors for all subjects stacked in the order of processing
    # (or in the order of arg list in multicore-mode) is called 'feature'
    (corr, featVec, feature) = Correlater(stackArray)
...
def MaskWrapper(maskArguments, maskNo=0):
    ...
    print 'Starting Multicore Processing with', str(numberCores), 'cores.'
    start = time.time()
...
def Main(mPath,
    ...
    # read from subjectList
    batch = open(subjectList)
    ages = np.array([], dtype='float32')
    subList = []
    ...
    storageDict['condition'] = conditionName

    # prepare the filename
    prefix = 'preprocessed_'
    suffix = conditionName
    ...
    if saveFiles == 1:
        ...
        print '\n########## '
        print 'Nothing will be saved because \'saveFiles\' has not been set. '
        print '########## '
